...
class TestBigMath < Test::Unit::TestCase
  ...
  def test_gamma
    ...
    assert_converge_in_precision {|n| gamma(BigDecimal("1e15") + BigDecimal("0.5"), n) }
    ...
    # x extremely close to an interpolation node stresses the batch_prod cancellation in BSGS
    assert_converge_in_precision {|n| gamma(BigDecimal(5) + BigDecimal("1e-40"), n) }
    # x closer to a node than the working precision must not produce a zero node distance
    assert_converge_in_precision {|n| gamma(BigDecimal(5) + BigDecimal("1e-300"), n) }
    assert_converge_in_precision {|n| gamma(BigDecimal("-3") - BigDecimal("1e-2000"), n) }
    ...
    # crosses the multipoint engine threshold (coefficient domain at 4000, value domain at 8000)
    assert_converge_in_precision([4000, 8000]) {|n| gamma(BigDecimal(1).div(3, n * 2), n) }
  end
  ...
  def test_lgamma
    ...
    # x closer to a node than the working precision must not produce a zero node distance
    assert_converge_in_precision {|n| lgamma(BigDecimal(5) + BigDecimal("1e-300"), n).first }
    # x closer to 1 or 2 than any tested precision takes the linear interpolation path
    assert_converge_in_precision {|n| lgamma(BigDecimal(1) + BigDecimal("1e-2000"), n).first }
    assert_converge_in_precision {|n| lgamma(BigDecimal(2) + BigDecimal("1e-2000"), n).first }
  end
