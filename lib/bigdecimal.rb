  ...
  module Internal # :nodoc:
    ...
    def self.taylor_sum_binary_splitting(x, ds, prec) # :nodoc:
      ...
      while fs.size > 1
        ...
        # to:   (1 + ((a0+x)*b1+x*b0)/(a1*b1) + (x*x)/(a1*b1) * rest)
...
module BigMath
  ...
  def self.log(x, prec)
    ...
    (0..prec2.bit_length).reverse_each do |i|
      p = [(prec2 >> i) + 2, BigDecimal.double_fig].max
  ...
  def self.exp(x, prec)
    ...
    # Each term is calculated using divide and rationalize method.
    x = x.mult(1, prec2)
    n = 2
    y = BigDecimal(1)
